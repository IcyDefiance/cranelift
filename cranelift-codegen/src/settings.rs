...
pub enum SetError {
    ...
    #[cfg_attr(feature = "std", error("No existing setting named '{0}'"))]
    BadName(String),

    /// Type mismatch for setting (e.g., setting an enum setting as a bool).
    #[cfg_attr(feature = "std", error("Trying to set a setting with the wrong type"))]
    BadType,

    /// This is not a valid value for this setting.
    #[cfg_attr(feature = "std", error("Unexpected value for a setting, expected {0}"))]
    BadValue(String),
}
